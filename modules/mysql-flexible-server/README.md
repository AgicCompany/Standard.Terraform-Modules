...
| <a name="input_enable_public_access"></a> [enable\_public\_access](#input\_enable\_public\_access) | Allow public network access to the server. Default false (private). Forced off (computed) when delegated\_subnet\_id is set, since VNet-integrated servers are inherently private. | `bool` | `false` | no |
