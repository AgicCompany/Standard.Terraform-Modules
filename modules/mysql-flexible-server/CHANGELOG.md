...
- `diagnostic_settings` and `administrator_password` validations crashed `terraform plan` on Terraform 1.10–1.12 (which do not short-circuit `||` in validation conditions) whenever a sink was set without `log_analytics_destination_type`, or whenever `administrator_password` was left `null`. Rewritten as nested ternaries; no interface or behavior change.
