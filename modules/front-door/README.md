...
|------|-------------|------|---------|:--------:|
| <a name="input_custom_domains"></a> [custom\_domains](#input\_custom\_domains) | Map of custom domains to attach to the Front Door profile. Key is used as the domain resource name. | <pre>map(object({<br/>    hostname         = string<br/>    certificate_type = optional(string, "ManagedCertificate")<br/>    secret_id        = optional(string)<br/>  }))</pre> | `{}` | no |
