...
variable "enabled_log_categories" {
  ...
  validation {
    condition     = var.enabled_log_categories == null ? true : length(var.enabled_log_categories) > 0
...
variable "metric_categories" {
  ...
  validation {
    condition     = var.metric_categories == null ? true : length(var.metric_categories) > 0
