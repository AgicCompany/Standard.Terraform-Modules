...
- Validations on `maintenance_window`, `maintenance_window_auto_upgrade` and `maintenance_window_node_os` crashed `terraform plan` on Terraform 1.10–1.12 when the variable was set to `null` (documented as supported). Rewritten as nested ternaries; no interface or behavior change.
