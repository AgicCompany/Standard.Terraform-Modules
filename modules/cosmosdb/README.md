...
```hcl
module "cosmosdb" {
  source = "git::https://github.com/AgicCompany/Standard.Terraform-Modules.git//modules/cosmosdb?ref=cosmosdb/v2.0.0"
...
| <a name="input_enable_virtual_network_filter"></a> [enable\_virtual\_network\_filter](#input\_enable\_virtual\_network\_filter) | Enable VNet filtering on the account so `virtual_network_rule` blocks are honored. Default false preserves prior behavior; set true to restrict access by subnet. | `bool` | `false` | no |
