...
Use terraform-docs to generate Inputs and Outputs tables. Place this config file at the `terraform-modules` repository root as `.terraform-docs.yml` â€” it applies to all modules in the repo:
...
| Bump | When | Examples |
|------|------|----------|
| **MAJOR** | Breaking change â€” consumers must modify code | Rename variable, remove output, change default behavior |
...
**Note on default changes:** Changing a default value is a breaking change even if it improves security. Consumers may have relied on the old behavior. For example, changing `public_network_access_enabled` from `true` to `false` requires a MAJOR bump â€” existing deployments would suddenly lose public access.
...
â”œâ”€â”€ variables.tf      # Input variables (grouped as per section 2)
â”œâ”€â”€ locals.tf         # Local values (keep even if empty)
â”œâ”€â”€ data.tf           # Data sources (keep even if empty)
â”œâ”€â”€ main.tf           # Resource definitions
â”œâ”€â”€ outputs.tf        # Output definitions (as per section 4)
â”œâ”€â”€ examples/
â”‚   â”œâ”€â”€ basic/
â”‚   â”‚   â”œâ”€â”€ main.tf
â”‚   â”‚   â””â”€â”€ README.md
...
â””â”€â”€ README.md         # Module documentation (as per section 5)
```
...
Modules use the default provider configuration. They do not declare `provider` blocks internally â€” this would prevent consumers from passing custom configurations.
...
This is an advanced pattern â€” most modules won't need it.
